...
it('when the system date and the date to format are not the same day formats as "16/06/2019"', () => {
  const June = 5; //js Date object month is indexed from 0
  const systemDateTime = new Date(2019, June, 15, 10, 5).getTime();
  const dateTimeToFormat = new Date(2019, June, 16, 12, 50).getTime(); //Same day different time
  expect(format(dateTimeToFormat, systemDateTime)).toBe('16/06/2019');
});

it('when the system date and the date to format are not the same day formats as "15/12/2018"', () => {
  const December = 11; //js Date object month is indexed from 0
